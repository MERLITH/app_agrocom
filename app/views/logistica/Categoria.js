...
let Componente = {
                ...
                <div class="col-md-8">
                    <h4 class="fw-bold py-0 mb-4">
                        ...
                        <small>
                            ...
                            <a href="javascript:" class="icon-home text-primary">Categorías</a>&nbsp;
                        </small>
                    </h4>
                </div>
    ...
    /************ */
    eventos: ()=> {
        DOM.find('input[name="image"]').change(function(e) {
            e.stopImmediatePropagation();
    ...
    datatable: async function() {
        ...
        var oData = await $.getData('logistica/tipo_articulo');

        var htmlTable = '<table class="datatables-users table tblRegistros"></table>';
        DOM.find('#Cnt_tblRegistros').html(htmlTable);

        var oColums = [
            { "data": "name", "title": "NOMBRE", "class": "text-left", ordenable: true },
            { "data": "description", "title": "DESCRIPCIÓN", "class": "text-left", ordenable: true },
            {
    ...
    validarForm: function (idform, accion = 'nuevo') {
        ...
        var arrayFields = [
            {field: 'name', message: 'Por favor, Escriba el nombre'},
            {field: 'description', message: 'Por favor, Escriba el descripción'},
            {field: 'image', message: 'Por favor, Escriba el imagen'},
        ];
        ...
        var chtml = `
            <form name="save" id="formSave">
                <div class="row mt-2">
                    ...
                    <div class="col-12">
                        <label class="form-label">Imagen <b class="text-danger">*</b></label>
                        <input type="file" name="image" class="form-control" autocomplete="off" accept=".jpg,.jpeg,.png,.gif">
                    </div>
                    <div class="col-12 text-center">
                        <img name="imagen" style="max-width:50%;" class="img_rectangle">
                    </div>
                </div>
    ...
    edit: async function(data) {
        ...
        this.id = data.id;
        this.action_submit = accion;
        this.imagen_anterior = data.image;
        ...
        form.find('img[name="imagen"]').attr('src', BASE_FILES+'images/'+data.image);
    },
    ...
    submit: function() {
        ...
        axios({
            method: 'post',
            url: BASE_API + 'logistica/tipo_articulo/' + this.action_submit,
            data: formData
        })
